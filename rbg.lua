...
local function autoGreenShot()
    ...
    -- RB5 W5 reality: the meter RESETS on shot start (LO confirmed raw
    -- climbed from 0 during shot). So we read raw value DIRECTLY as the
    -- target — no delta math.
    ...
    while not state.closed do
        ...
        if raw >= CONFIG.target and raw <= CONFIG.overchargeCeiling then
            for _ = 1, 6 do pcall(function() keyrelease(CONFIG.shootKey) end) end
            releaseAt = raw
            break
        end

        -- safety: if meter starts falling significantly from its peak,
        -- treat the peak as our release moment
        if peakSeen > 0.3 and raw < peakSeen * 0.6 then
            for _ = 1, 6 do pcall(function() keyrelease(CONFIG.shootKey) end) end
            releaseAt = peakSeen
            break
        end
