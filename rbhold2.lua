...
local CONFIG = {
    target            = 0.970,   -- release when raw meter crosses this · green band 0.93-1.05
...
local function autoGreenShot()
    ...
    while not state.closed do
        ...
        if sawReset then
            -- Tiny safety floor to avoid catching the very-low reset transient
            local heldFor = tick() - resetAt
            if heldFor >= 0.15 then
                -- Release the instant raw crosses target going UP
                if raw >= CONFIG.target then
