...
hud.lineHint   = mkLine(140, "TAP E briefly · T=toggle [ ]=tune `=record M=diag N=close")
...
local function autoGreenShot()
    ...
    -- RB5 W5 quirk: meter has a RESIDUAL value (e.g. 2.000) at rest from
    -- the previous shot. So we can't release on raw>=target directly —
    -- we have to wait for the meter to RISE FROM A RECENT LOW above target.
    -- That handles both: meter resets on shot start, AND meter that
    -- climbs from residual.
    --
    -- ALSO: we no longer call keypress(E) — LO is pressing E himself.
    -- We just monitor and fire keyrelease at the right moment.

    local startedAt = tick()
    local releaseAt = 0
    local meterHistory = {}
    local sawReset = false   -- did the meter drop to a low value (fresh shot)?
    ...
    while not state.closed do
        ...
        table.insert(meterHistory, raw)
        if #meterHistory > 40 then table.remove(meterHistory, 1) end

        -- recent low across last ~200ms
        local recentLow = raw
        for _, v in ipairs(meterHistory) do
            if v < recentLow then recentLow = v end
        end

        -- Did the meter dip to a low value at any point? That's our "fresh shot" signal
        if recentLow < 0.3 then sawReset = true end

        -- Primary release: meter has risen from a low to target band
        if sawReset and raw >= CONFIG.target and raw <= CONFIG.overchargeCeiling then
